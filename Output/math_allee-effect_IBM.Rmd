...
$$
  \phi_N(i,j,t) =
    \begin{cases}
      1 & \text{if } S_i \ne \text{larva}\\
      (\frac{1}{1+\beta,c_{j,t}}) & \text{if } S_i = \text{larva}
    \end{cases}       
$$

where $c_{j,t}$ is the number of larva in each patch, and $\beta$ is the strength of density dependence in the targeted stage and can be estimated from experimental studies. This formulation assumes a decrease in survival as larval density increases, and reflects competition for limited resources (Bellows, 1981; Walker et al., 2020).
