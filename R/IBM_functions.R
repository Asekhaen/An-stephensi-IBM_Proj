  ...
  for (i in seq_along(pop_patches)) {
    ...
    if (nrow(fem) > 0 && nrow(male) > 0)
      ...
      for (j in 1:nrow(fem)) {
        ...
        if (rbinom(1, 1, (nrow(male)/(100 + nrow(male)))) == 1 && rbinom(1, 1, bloodmeal_prob) == 1) {   #  (nrow(male)/(100 + nrow(male)))) is the mating probability which increases as male population increases (North and Godfray; Malar J (2018) 17:140) 
          population 
